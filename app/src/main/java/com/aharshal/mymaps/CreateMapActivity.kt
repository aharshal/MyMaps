...
import com.aharshal.mymaps.databinding.ActivityCreateMapBinding
...
class CreateMapActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.miSave) {
            if (markers.isEmpty()) {
                Toast.makeText(this, "There must be atleast one market", Toast.LENGTH_LONG).show()
            }
        }
...
//            val marker = mMap.addMarker(MarkerOptions().position(latLng).title("Marker in Sydney").snippet("a cool snippet!"))
//            markers.add(marker)
        }
    ...
    private fun showAlertDialog(latLng: LatLng) {
        ...
        val dialog = AlertDialog.Builder(this).setTitle("Create a marker")
            .setView(placeFormView).setNegativeButton("Cancel", null)
